    ...
    async saveInvoiceToCloud(invoice: InvoiceData, pdfUrl?: string): Promise<boolean> {
        try {
            ...
            if (cleanContent.business) {
                const business = { ...cleanContent.business };

                // Ne garder que les URLs, pas les base64
                if (business.customHeaderImage && this.isDataUrl(business.customHeaderImage)) {
                    // Si c'est encore en base64, on le garde pour compatibilité
                    // mais idéalement ça devrait déjà être une URL
                }

                if (business.signatureUrl && this.isDataUrl(business.signatureUrl)) {
                    // Idem pour la signature
                }

                cleanContent.business = business;
            }
